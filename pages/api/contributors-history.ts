...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // Also, join with the "conlist" table to fetch the wallet associated with each contributor
      query = `SELECT v.contributor_name, c.wallet, SUM(v.points) AS points_sum 
               FROM voting v
               JOIN conlist c ON v.contributor_name = c.name
               WHERE v.phase_id = ?
               GROUP BY v.contributor_name, c.wallet`;
